...
- Fixed Antigravity IDE auto-accept for Monaco editor diff widgets & review bars (`Accept Changes`, `Accept Alt+Enter`, `Accept all`): Expanded `getScanRoots` beyond the chat sidepanel to include `.monaco-editor`, `.zone-widget`, `.inline-chat-widget`, and `[class*="diff"]`, and resolved a template literal regex newline evaluation error (`_shortcutSuffixRegex`) that prevented the DOM MutationObserver from initializing on open windows.
- Enhanced agent response formatting: Stripped all internal thinking, `Worked for Xs`, tool execution logs (`Ran command`, `Ran tools`, `Explored files`), and thought blocks (`<thought>...</thought>`) from automatic Telegram responses, ensuring only the pure, nicely formatted final answer is sent. Full thought traces remain accessible exclusively via `/latest`.
- Added interactive `/agents` menu & instant UUID switching: Upgraded `/agents` from a wall of text to an interactive inline keyboard UI with quick recent chat buttons, project folders, fast keyword search (`/agents <keyword>`), and direct 2ms UUID navigation (`switchAgentThread` with `targetThreadId`).
