...
### Fixes
- Fixed Standalone App (2.0) `/agents` list and thread switching: Updated `StandaloneDriver` (`getListAgentThreadsScript`, `getSwitchThreadScript`, `getActiveThreadInfoScript`) and locators to navigate the modern sidebar hierarchy (`button[class*='headerbtn']` workspace sections and `a[href*='/c/']` conversation links) and extract active workspace/thread metadata cleanly.
